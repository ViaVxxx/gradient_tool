<template>
  <main class="flex-1 h-full bg-checkerboard relative overflow-hidden">
    ...
    <!-- 底部悬浮工具栏 -->
    <div
      class="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-20 animate-slide-in-bottom transition-all duration-300"
      :class="{ 'opacity-30 hover:opacity-100': isPureMode }">
      <div class="smart-toolbar rounded-xl px-3 md:px-6 py-3 flex items-center gap-2 md:gap-4 shadow-2xl flex-wrap justify-center max-w-[90vw]">

        <!-- 纯净模式 (桌面端显示) -->
        <button
          @click="togglePureMode"
          class="hidden md:flex items-center justify-center h-10 w-10 rounded-xl text-white/90 hover:text-white hover:bg-white/20 transition-all duration-200"
          title="纯净模式">
        ...
        <!-- 预览模式切换 -->
        <div class="flex items-center bg-black/20 rounded-xl p-1 backdrop-blur-sm h-10">
          <button
            @click="previewMode = 'css'"
            :class="[
              'px-2 md:px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200',
        ...
        <button
          ...
          class="h-10 w-10 flex items-center justify-center rounded-xl text-white/90 hover:text-white hover:bg-white/20 transition-all duration-200"
          title="随机生成">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          class="h-10 px-4 md:px-6 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white text-sm font-semibold transition-all duration-200 disabled:opacity-50 shadow-lg hover:shadow-xl hover:scale-105">
        ...
        <button
          ...
          class="h-10 w-10 flex items-center justify-center rounded-xl text-white/90 hover:text-white hover:bg-white/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          title="导出图像">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
